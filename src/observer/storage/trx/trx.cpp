...
{
    if (common::is_blank(name) || 0 == strcasecmp(name, "vacuous")) { return new VacuousTrxKit(); }

    if (0 == strcasecmp(name, "mvcc")) { return new MvccTrxKit(); }
...
RC TrxKit::init_global(const char* name)
{
    ...
    RC rc = trx_kit->init();
    if (rc == RC::SUCCESS) { global_trxkit = trx_kit; }
    return rc;
}
