...
RC PersistHandler::remove_file(const char* file_name)
{
    ...
    if (file_name != nullptr)
    {
        if (remove(file_name) == 0) { LOG_INFO("Successfully remove file %s.", file_name); }
        else
        {
    ...
    else if (!file_name_.empty())
    {
        ...
        {
            if (remove(file_name_.c_str()) == 0) { LOG_INFO("Successfully remove file %s.", file_name_.c_str()); }
            else
            {
...
RC PersistHandler::write_file(int size, const char* data, int64_t* out_size)
{
    ...
    else
    {
        ...
        if (out_size != nullptr) { *out_size = write_size; }
    }
...
RC PersistHandler::read_at(uint64_t offset, int size, char* data, int64_t* out_size)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (read_size == 0) { LOG_TRACE("read file touch the end. file name=%s", file_name_.c_str()); }
            else if (read_size < 0)
            {
            ...
            else if (out_size != nullptr) { *out_size = read_size; }
        }
    }
