...
RC PredicateRewriteRule::rewrite(std::unique_ptr<LogicalOperator>& oper, bool& change_made)
{
    ...
    if (child_opers.size() != 1) { return RC::SUCCESS; }

    auto& child_oper = child_opers.front();
    if (child_oper->type() != LogicalOperatorType::PREDICATE) { return RC::SUCCESS; }

    std::vector<std::unique_ptr<Expression>>& expressions = child_oper->expressions();
    if (expressions.size() != 1) { return RC::SUCCESS; }

    std::unique_ptr<Expression>& expr = expressions.front();
    if (expr->type() != ExprType::VALUE) { return RC::SUCCESS; }
    ...
    if (true == bool_value)
    {
        ...
        child_opers.clear();
        for (auto& grand_child_oper : grand_child_opers) { oper->add_child(std::move(grand_child_oper)); }
    }
    else { child_opers.clear(); }
