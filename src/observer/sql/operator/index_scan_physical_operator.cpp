...
{
    if (left_value) { left_value_ = *left_value; }
    if (right_value) { right_value_ = *right_value; }
}

RC IndexScanPhysicalOperator::open(Trx* trx)
{
    if (nullptr == table_ || nullptr == index_) { return RC::INTERNAL; }
...
RC IndexScanPhysicalOperator::next()
{
    ...
    while (RC::SUCCESS == (rc = index_scanner_->next_entry(&rid)))
    {
        ...
        if (rc == RC::RECORD_INVISIBLE) { continue; }
        else { return rc; }
    }
